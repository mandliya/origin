...
namespace origin
{
    ...
    /*
     * Non-Mutable Pairing Heap Implementation 
     */
    template <class T,
             class Compare>
    class pairing_heap
    {
       private:
          ...
          /*
           * top_ - index of the root element
           */ 
           ...
           * size_type y : Index of the root element of the heap that takes a new left child 
           * Output:
           * Merges the heap pointed to by x in the main heap pointed to by y
           * Return Value:
           * None       
           */
          void merge (size_type index1, size_type index2)
       ...
       public:
          /*
           * pairing_heap: Default constructor
           * Input: 
           * None
           * Output:
           * Return Value:
    ...
    template <class T, 
              class Compare>
    void pairing_heap<T, Compare>::push(const value_type& d)
    {
        //make a single node heap of the new element
        pairing_heap_node obj;
        size_type index;
    ...
    template <class T, 
              class Compare>
    void pairing_heap<T, Compare>::pop()
    {
        if (top_ == size_type(-1))
            return;
    ...
    template <class T, 
              class Compare>
       template<typename Char, typename Traits>
    void pairing_heap<T, Compare>::print(std::basic_ostream<Char, Traits>& os)
    {
        if (top_ != size_type(-1)) {
            print_recur(top_, os);
            os << std::endl;
